...
// supported) for use in OpenGL. Image is flipped when it's loaded as DX images
...
void CDDSImage::create_textureFlat(unsigned int format, unsigned int components, const CTexture &baseImage) {
	...
	DEBUG_ASSERT(baseImage.get_depth() == 1);
...
void CDDSImage::create_textureCubemap(unsigned int format, unsigned int components,
	...
	DEBUG_ASSERT(format != 0);
	DEBUG_ASSERT(components != 0);
	DEBUG_ASSERT(positiveX.get_depth() == 1);

	// verify that all dimensions are the same 
	DEBUG_ASSERT(same_size(positiveX, negativeX));
	DEBUG_ASSERT(same_size(positiveX, positiveY));
	DEBUG_ASSERT(same_size(positiveX, negativeY));
	DEBUG_ASSERT(same_size(positiveX, positiveZ));
	DEBUG_ASSERT(same_size(positiveX, negativeZ));
...
bool CDDSImage::load(string filename, bool flipImage) {
	DEBUG_ASSERT(filename.length() != 0);
...
void CDDSImage::write_texture(const CTexture &texture, FILE *fp) {
	DEBUG_ASSERT(get_num_mipmaps() == texture.get_num_mipmaps());
	...
	for (unsigned int i = 0; i < texture.get_num_mipmaps(); i++) {
		...
		r = fwrite(mipmap, 1, mipmap.get_size(), fp);
	}
}

bool CDDSImage::save(std::string filename, bool flipImage) {
	DEBUG_ASSERT(m_valid);
	DEBUG_ASSERT(m_type != TextureNone);
	...
	// write dds header
	r = fwrite(&ddsh, 1, sizeof(DDS_HEADER), fp);
...
bool CDDSImage::upload_texture1D() {
	DEBUG_ASSERT(m_valid);
	DEBUG_ASSERT(!m_images.empty());

	const CTexture &baseImage = m_images[0];

	DEBUG_ASSERT(baseImage.get_height() == 1);
	DEBUG_ASSERT(baseImage.get_width() > 0);
...
bool CDDSImage::upload_texture2D(unsigned int imageIndex, GLenum target) {
	DEBUG_ASSERT(m_valid);
	DEBUG_ASSERT(!m_images.empty());
	DEBUG_ASSERT(imageIndex < m_images.size());
	DEBUG_ASSERT(m_images[imageIndex]);

	const CTexture &image = m_images[imageIndex];

	DEBUG_ASSERT(image.get_height() > 0);
	DEBUG_ASSERT(image.get_width() > 0);
	DEBUG_ASSERT(target == GL_TEXTURE_2D || target == GL_TEXTURE_RECTANGLE_NV ||
	...
	if (is_compressed()) {
		...
		if (glCompressedTexImage2DARB == NULL) {
			GET_EXT_POINTER(glCompressedTexImage2DARB, PFNGLCOMPRESSEDTEXIMAGE2DARBPROC);
        }
...
bool CDDSImage::upload_textureCubemap() {
	DEBUG_ASSERT(m_valid);
	DEBUG_ASSERT(!m_images.empty());
	DEBUG_ASSERT(m_type == TextureCubemap);
	DEBUG_ASSERT(m_images.size() == 6);
...
void CDDSImage::flip(CSurface& surface) {
	...
	if (!is_compressed()) {
		DEBUG_ASSERT(surface.get_depth() > 0);
