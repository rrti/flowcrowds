...
#ifndef FLOWCROWDS_NVDDS_HDR
#define FLOWCROWDS_NVDDS_HDR
	...
	class CTexture: public CSurface {
		...
		public:
			...
			inline const CSurface &get_mipmap(unsigned int index) const {
				DEBUG_ASSERT(!m_mipmaps.empty());
				DEBUG_ASSERT(index < m_mipmaps.size());
	...
	class CDDSImage {
		public:
			...
			inline const CSurface &get_mipmap(unsigned int index) const {
				DEBUG_ASSERT(m_valid);
				DEBUG_ASSERT(!m_images.empty());
				DEBUG_ASSERT(index < m_images[0].get_num_mipmaps());
