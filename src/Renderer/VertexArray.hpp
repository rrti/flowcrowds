#ifndef FLOWCROWDS_VERTEXARRAY_HDR
#define FLOWCROWDS_VERTEXARRAY_HDR
...
#ifdef DEBUG
	#define ASSERT_SIZE(x) DEBUG_ASSERT(drawArraySize >= (drawArrayPos + x));
#else
...
void VertexArray::EndStripQ() {
	DEBUG_ASSERT(stripArraySize >= stripArrayPos + 1);
