...
void CSimThread::SimCommand(NetMessage& m) {
	...
	switch (simCommandID) {
		case COMMAND_CREATE_SIMOBJECT: {
			DEBUG_ASSERT(!m.End()); m >> objectDefID;
			DEBUG_ASSERT(!m.End()); m >> objectPos.x;
			DEBUG_ASSERT(!m.End()); m >> objectPos.y;
			DEBUG_ASSERT(!m.End()); m >> objectPos.z;
			DEBUG_ASSERT(!m.End()); m >> objectDir.x;
			DEBUG_ASSERT(!m.End()); m >> objectDir.y;
			DEBUG_ASSERT(!m.End()); m >> objectDir.z;
		...
		case COMMAND_MOVE_SIMOBJECT: {
			...
			DEBUG_ASSERT(!m.End()); m >> objectPos.y;
			DEBUG_ASSERT(!m.End()); m >> objectPos.z;
			DEBUG_ASSERT(!m.End()); m >> queueCommand;
			DEBUG_ASSERT(!m.End());
