...
void CMapInfo::FreeInstance(const CMapInfo* mi) {
	delete (const_cast<CMapInfo*>(mi));
}
...
CMapInfo::CMapInfo(const std::string& mapName) {
	...
	*/
	...
	light.groundAmbientColor  = mapTable->GetVec<vec4f>("groundAmbientColor", 4) / 100.0f;
	light.groundDiffuseColor  = mapTable->GetVec<vec4f>("groundDiffuseColor", 4) / 100.0f;
	light.groundSpecularColor = mapTable->GetVec<vec4f>("groundSpecularColor", 4) / 100.0f;
	light.sunDir              = mapTable->GetVec<vec4f>("sunDir", 4);
...
void CMapInfo::ReadAtmosphere() {
	...
	atmo.fogColor     = atmoTable.GetFloat3("fogColor", vec3f(0.7f, 0.7f, 0.8f));
	atmo.skyColor     = atmoTable.GetFloat3("skyColor", vec3f(0.1f, 0.15f, 0.7f));
	atmo.sunColor     = atmoTable.GetFloat3("sunColor", vec3f(1.0f, 1.0f, 1.0f));
	atmo.cloudColor   = atmoTable.GetFloat3("cloudColor", vec3f(1.0f, 1.0f, 1.0f));
...
void CMapInfo::ReadLight() {
	...
	light.groundAmbientColor  = lightTable.GetFloat3("groundAmbientColor", vec3f(0.5f, 0.5f, 0.5f));
	light.groundSunColor      = lightTable.GetFloat3("groundDiffuseColor", vec3f(0.5f, 0.5f, 0.5f));
	light.groundSpecularColor = lightTable.GetFloat3("groundSpecularColor", vec3f(0.1f, 0.1f, 0.1f));
	light.groundShadowDensity = lightTable.GetFloat("groundShadowDensity", 0.8f);

	light.unitAmbientColor  = lightTable.GetFloat3("unitAmbientColor", vec3f(0.4f, 0.4f, 0.4f));
	light.unitSunColor      = lightTable.GetFloat3("unitDiffuseColor", vec3f(0.7f, 0.7f, 0.7f));
	light.specularSunColor  = lightTable.GetFloat3("unitSpecularColor", light.unitSunColor);
...
void CMapInfo::ReadWater() {
	...
	water.absorb    = wt.GetFloat3("absorb",    vec3f(0.0f, 0.0f, 0.0f));
	water.baseColor = wt.GetFloat3("baseColor", vec3f(0.0f, 0.0f, 0.0f));
	water.minColor  = wt.GetFloat3("minColor",  vec3f(0.0f, 0.0f, 0.0f));
	...
	water.planeColor = wt.GetFloat3("planeColor", vec3f(0.0f, 0.4f, 0.0f));
	hasWaterPlane    = wt.KeyExists("planeColor");

	water.surfaceColor  = wt.GetFloat3("surfaceColor", vec3f(0.75f, 0.8f, 0.85f));
	water.surfaceAlpha  = wt.GetFloat("surfaceAlpha",  0.55f);
	water.diffuseColor  = wt.GetFloat3("diffuseColor",  vec3f(1.0f, 1.0f, 1.0f));
...
bool CMapInfo::GetStartPos(int team, vec3f& pos) const {
