...
#define SIMOBJECT_ALLOW_INPLACE_TURNS           1
...
void CCGrid::Init(unsigned int downScaleFactor, ICallOutHandler* coh) {
	DEBUG_ASSERT(downScaleFactor >= 1);
	...
	printf("\tSIMOBJECT_ALLOW_INPLACE_TURNS:           %d\n", SIMOBJECT_ALLOW_INPLACE_TURNS);
	printf("\n");
	...
	for (unsigned int y = 0; y < numCellsZ; y++) {
		for (unsigned int x = 0; x < numCellsX; x++) {
			...
			if (x > 0) {
				...
				DEBUG_ASSERT(cellIdx < currCells.size());
	...
	DEBUG_ASSERT(currCells.size() == numCells);
	DEBUG_ASSERT(prevEdges.size() == numEdges);
	...
	if (mFlatTerrain) {
		DEBUG_ASSERT((mMaxTerrainSlope - mMinTerrainSlope) < EPSILON);
	}
}
...
void CCGrid::UpdateGroupPotentialField(unsigned int groupID, const std::set<unsigned int>& goalIDs, const std::set<unsigned int>& objectIDs) {
	DEBUG_ASSERT(!goalIDs.empty());
	DEBUG_ASSERT(mCandidates.empty());
	...
	// of the update so that each sim-object location update
	// still reads from the correct buffer
	...
	DEBUG_ASSERT(numInfinitePotentialCases == 0 && numIllegalDirectionCases == 0);
}
...
void CCGrid::UpdateCandidates(unsigned int groupID, const Cell* parent) {
	...
	for (unsigned int i = 0; i < parent->numNeighbors; i++) {
		...
		DEBUG_ASSERT((int(undefinedX) + int(undefinedY)) < 2);
		...
		if (!undefinedX && !undefinedY) {
			...
			DEBUG_ASSERT(dirValid[DIR_N] || dirValid[DIR_S]);
			DEBUG_ASSERT(dirValid[DIR_E] || dirValid[DIR_W]);
			DEBUG_ASSERT(minPotCellPtrX != NULL && minPotCellPtrY != NULL);
			...
			if (undefinedX) {
				...
				DEBUG_ASSERT(dirValid[DIR_N] || dirValid[DIR_S]);
				DEBUG_ASSERT(!undefinedY);
				DEBUG_ASSERT(minPotCellPtrY != NULL);
			...
			if (undefinedY) {
				...
				DEBUG_ASSERT(dirValid[DIR_E] || dirValid[DIR_W]);
				DEBUG_ASSERT(!undefinedX);
				DEBUG_ASSERT(minPotCellPtrX != NULL);
...
float CCGrid::Potential2D(const float p1, const float c1, const float p2, const float c2) const {
	...
	DEBUG_ASSERT(c1s_plus_c2s > 0.0f);
...
bool CCGrid::UpdateSimObjectLocation(unsigned int groupID, unsigned int objectID, unsigned int objectCellID) {
	...
	DEBUG_ASSERT_MSG(!(std::isnan(objectCellVel.x) || std::isnan(objectCellVel.y) || std::isnan(objectCellVel.z)), "Inf velocity-field for cell <%u,%u>", objectCell->x, objectCell->y);
	DEBUG_ASSERT_MSG(!(std::isinf(objectCellVel.x) || std::isinf(objectCellVel.y) || std::isinf(objectCellVel.z)), "NaN velocity-field for cell <%u,%u>", objectCell->x, objectCell->y);
	...
	if (objectCellVel.sqLen2D() > EPSILON) {
...

			{
				...
				const float turnAngleRad = DEG2RAD(turnAngleDeg);

				const bool  instaTurn = (std::fabs(deltaGlobalAngleRad) <= turnAngleRad);
				const float turnAngle = instaTurn? deltaGlobalAngleRad: turnAngleRad;
				const float turnSign  = (deltaGlobalAngleRad > 0.0f)? 1.0f: -1.0f;

				if (std::fabs(turnAngle) > MIN_ANGLE_RAD || turnAngleRad <= MIN_ANGLE_RAD) {
